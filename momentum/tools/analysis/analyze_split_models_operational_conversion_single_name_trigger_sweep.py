...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_single_name_trigger_sweep"
...
def _compose_variant_name(top1_threshold: float, trim_fraction: float) -> str:
    return (
        f"{BASE_VARIANT}_singtrig_t1{int(round(top1_threshold * 100)):02d}"
...
def _patch_single_name_trigger_kr_etf_only(top1_threshold: float, trim_fraction: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        top1_weight = float(top1["TargetWeight"].sum())
        if top1_weight < top1_threshold:
            return out
        ...
        if dominant_sector_weight < SECTOR_CONCENTRATION_THRESHOLD:
            return out
...
def _compose_patch(top1_threshold: float, trim_fraction: float):
    base_patch = _patch_tail_release_custom(top2_share=0.25, penalty_start=0.35, penalty_floor=0.25)
    trigger_patch = _patch_single_name_trigger_kr_etf_only(top1_threshold, trim_fraction)
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Single Name Trigger Sweep",
        "",
        "## Purpose",
        "",
        "- test a new trigger shape inside the current representative family",
        "- trigger only when a single top US name and a target sector are both highly concentrated",
        "",
        "## Current Read",
        "",
        f"- representative benchmark: `{summary['representative_variant']}`",
        f"- best single-name trigger point: `{summary['best_variant']}`",
        f"- best single-name MDD: `{_pct(summary['best_mdd'])}`",
        f"- best single-name CAGR: `{_pct(summary['best_cagr'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Top1 Trigger | Trim | Switch Count | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | {_pct(row['Top1Threshold'])} | {_pct(row['TrimFraction'])} | "
...
def main() -> None:
    ...
    for top1_threshold, trim_fraction in [
        ...
        variant_name = _compose_variant_name(top1_threshold, trim_fraction)
        variant = replace(strongest, name=variant_name)
        result, switch_count = _run_with_baseline_switch_carry(
            variant=variant,
            fallback_variant=baseline,
            redistribution_patch=_compose_patch(top1_threshold, trim_fraction),
        ...
        summary["Top1Threshold"] = top1_threshold
    ...
    compare.to_csv(
        OUTPUT_DIR / "single_name_trigger_sweep_compare.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    summary = {
        ...
        "best_top1_threshold": None if pd.isna(best["Top1Threshold"]) else float(best["Top1Threshold"]),
        ...
        "matches_representative": matches_representative,
        "ranked_rows": compare.to_dict(orient="records"),
    }
    if matches_representative:
        summary["verdict"] = (
            f"the single-name trigger does not produce a new representative challenger. "
            f"`{summary['best_variant']}` matches the current representative at "
            f"CAGR {_pct(summary['best_cagr'])} and MDD {_pct(summary['best_mdd'])}."
        )
    elif best["MDD"] > representative_mdd + MDD_TOL and best["CAGR"] >= representative_cagr - 1e-12:
        summary["verdict"] = (
            f"the single-name trigger finds a representative challenger. `{summary['best_variant']}` moves to "
        ...
        summary["verdict"] = (
            f"the single-name trigger creates a growth-heavy alternative, not a representative replacement. "
            f"`{summary['best_variant']}` lifts CAGR to {_pct(summary['best_cagr'])} but gives back MDD to "
            f"{_pct(summary['best_mdd'])} versus representative {_pct(representative_mdd)}."
        )
    elif best["MDD"] > base_summary["MDD"] + MDD_TOL:
        summary["verdict"] = (
            f"the single-name trigger improves drawdown versus base but not enough to replace the representative. "
            f"`{summary['best_variant']}` reaches {_pct(summary['best_mdd'])} MDD with {_pct(summary['best_cagr'])} CAGR."
        )
    else:
        summary["verdict"] = (
            f"the single-name trigger fails: no trigger point improves drawdown or quality versus `{BASE_VARIANT}`."
        )

    (OUTPUT_DIR / "single_name_trigger_sweep_summary.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
    (OUTPUT_DIR / "single_name_trigger_sweep_review.md").write_text(
