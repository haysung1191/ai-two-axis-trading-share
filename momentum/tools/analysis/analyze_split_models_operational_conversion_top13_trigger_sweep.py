...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_top13_trigger_sweep"
...
def _compose_variant_name(top1_threshold: float, top3_threshold: float, trim_fraction: float) -> str:
    return (
        f"{BASE_VARIANT}_top13trig"
        f"_t1{int(round(top1_threshold * 100)):02d}"
        f"_t3{int(round(top3_threshold * 100)):02d}"
...
def _patch_top13_trigger_kr_etf_only(top1_threshold: float, top3_threshold: float, trim_fraction: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        if float(top1["TargetWeight"].sum()) < top1_threshold:
            return out
        if float(top3["TargetWeight"].sum()) < top3_threshold:
            return out
...
def _compose_patch(top1_threshold: float, top3_threshold: float, trim_fraction: float):
    base_patch = _patch_tail_release_custom(top2_share=0.25, penalty_start=0.35, penalty_floor=0.25)
    trigger_patch = _patch_top13_trigger_kr_etf_only(top1_threshold, top3_threshold, trim_fraction)
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Top1 Top3 Trigger Sweep",
        "",
        "## Purpose",
        "",
        "- test a middle trigger family between the current concentration trigger and the single-name trigger",
        "- require both top1 concentration and top3 book concentration before trimming top2 into KR ETF recipients",
        "",
        "## Current Read",
        "",
        f"- representative benchmark: `{summary['representative_variant']}`",
        f"- best top1-top3 trigger point: `{summary['best_variant']}`",
        f"- best CAGR / MDD: `{_pct(summary['best_cagr'])}` / `{_pct(summary['best_mdd'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Top1 | Top3 | Trim | Switch Count | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | {_pct(row['Top1Threshold'])} | {_pct(row['Top3Threshold'])} | "
...
def main() -> None:
    ...
    for top1_threshold, top3_threshold, trim_fraction in [
        ...
        variant_name = _compose_variant_name(top1_threshold, top3_threshold, trim_fraction)
        variant = replace(strongest, name=variant_name)
        result, switch_count = _run_with_baseline_switch_carry(
            variant=variant,
            fallback_variant=baseline,
            redistribution_patch=_compose_patch(top1_threshold, top3_threshold, trim_fraction),
        ...
        summary["Top3Threshold"] = top3_threshold
    ...
    compare.to_csv(
        OUTPUT_DIR / "top13_trigger_sweep_compare.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    summary = {
        ...
        "best_top3_threshold": None if pd.isna(best["Top3Threshold"]) else float(best["Top3Threshold"]),
    ...
    if best["MDD"] > representative_mdd + MDD_TOL and best["CAGR"] >= representative_cagr - 1e-12:
        summary["verdict"] = (
            f"the top1-top3 trigger finds a representative challenger. `{summary['best_variant']}` reaches "
        ...
        summary["verdict"] = (
            f"the top1-top3 trigger improves drawdown beyond the representative but pays for it in CAGR. "
        ...
        summary["verdict"] = (
            f"the top1-top3 trigger becomes a growth-heavy alternative, not a representative replacement. "
        ...
        summary["verdict"] = (
            f"the top1-top3 trigger improves on the base anchor but does not replace the representative. "
            f"`{summary['best_variant']}` reaches {_pct(summary['best_cagr'])} / {_pct(summary['best_mdd'])}."
        )
    else:
        summary["verdict"] = (
            f"the top1-top3 trigger does not improve the current representative tradeoff. "
            f"`{summary['best_variant']}` does not beat `{representative_variant}` on the benchmark balance."
        )

    (OUTPUT_DIR / "top13_trigger_sweep_summary.json").write_text(
        json.dumps(summary, indent=2),
        encoding="utf-8",
    )
    (OUTPUT_DIR / "top13_trigger_sweep_review.md").write_text(
