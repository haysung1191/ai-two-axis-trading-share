...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_kr_etf_guard_sweep"
...
def _patch_kr_etf_guard(shift_fraction: float, threshold_gap: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        donor_mask = out["Market"].astype(str).eq("US") & ~out["Sector"].astype(str).eq("ETF")
        donors = out.loc[donor_mask].copy()
        if donors.empty:
            return out
        donor_weights = pd.to_numeric(donors["TargetWeight"], errors="coerce").fillna(0.0)
        released = float(donor_weights.sum() * shift_fraction)
        if released <= 0:
            return out

        out.loc[donors.index, "TargetWeight"] = donor_weights * (1.0 - shift_fraction)

        recipient_mask = out["Market"].astype(str).eq("KR") & out["Sector"].astype(str).eq("ETF")
        ...
        if recipient_total > 0:
            out.loc[recipients.index, "TargetWeight"] = recipient_weights + released * (recipient_weights / recipient_total)
        else:
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion KR ETF Guard Sweep",
        "",
        "## Purpose",
        "",
        "- test a weak-flow guard that shifts part of the US single-name sleeve into KR ETF sleeve",
        "- check whether explicit sleeve-level de-risking repairs drawdown better than name-level trims",
        "",
        "## Current Read",
        "",
        f"- redistribution base point: `{summary['base_variant']}`",
        f"- best KR-ETF guard point: `{summary['best_variant']}`",
        f"- best KR-ETF guard MDD: `{_pct(summary['best_mdd'])}`",
        f"- best KR-ETF guard CAGR: `{_pct(summary['best_cagr'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Bucket | Shift | Flow Gap | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | `{row['Bucket']}` | {_pct(row['ShiftFraction'])} | {_pct(row['ThresholdGap'])} | "
...
def main() -> None:
    ...
    for shift_fraction, threshold_gap in grid:
        variant_name = f"{BASE_VARIANT}_kretfguard_shift{int(round(shift_fraction * 100)):02d}_gap{int(round(threshold_gap * 100)):02d}"
        variant = replace(strongest, name=variant_name)
        result = _run_with_patch(
            variant,
            _compose_patches(base_patch, _patch_kr_etf_guard(shift_fraction, threshold_gap)),
            universe,
            price_cache,
            flow_cache,
            monthly_close,
        ...
        summary["ShiftFraction"] = shift_fraction
    ...
    base_summary["ShiftFraction"] = 0.0
    ...
    compare.to_csv(OUTPUT_DIR / "kr_etf_guard_sweep_compare.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "best_shift_fraction": float(best["ShiftFraction"]),
        "best_threshold_gap": float(best["ThresholdGap"]),
        "ranked_rows": compare.to_dict(orient="records"),
    }
    if best["MDD"] <= base_summary["MDD"] and best["CAGR"] <= base_summary["CAGR"] and best["Sharpe"] <= base_summary["Sharpe"]:
        summary["verdict"] = (
            f"the KR ETF sleeve guard axis fails: no guarded point improves drawdown or quality versus `{BASE_VARIANT}`."
        )
    elif abs(summary["best_mdd"] - float(base_summary["MDD"])) < 1e-12:
        summary["verdict"] = (
            f"the KR ETF sleeve guard improves quality but not drawdown. "
            f"`{summary['best_variant']}` keeps MDD flat at {_pct(summary['best_mdd'])} while moving CAGR to {_pct(summary['best_cagr'])}."
        )
    else:
        summary["verdict"] = (
            f"the KR ETF sleeve guard finds `{summary['best_variant']}` as the cleanest drawdown-repair point; "
            f"it moves MDD to {_pct(summary['best_mdd'])} with CAGR {_pct(summary['best_cagr'])}."
        )

    (OUTPUT_DIR / "kr_etf_guard_sweep_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "kr_etf_guard_sweep_review.md").write_text(_build_markdown(summary), encoding="utf-8")
