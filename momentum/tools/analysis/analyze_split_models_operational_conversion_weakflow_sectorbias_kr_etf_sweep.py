...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_weakflow_sectorbias_kr_etf_sweep"
...
def _compose_variant_name(sector_bias_min: float, trim_fraction: float) -> str:
    return (
        f"{BASE_VARIANT}_weakflowsectorkretf"
...
def _patch_weakflow_sectorbias_kr_etf_only(sector_bias_min: float, trim_fraction: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        if not _weak_flow_trigger(ranked, threshold_gap=THRESHOLD_GAP, sector_bias_min=sector_bias_min):
            return out
...
def _compose_patch(sector_bias_min: float, trim_fraction: float):
    base_patch = _patch_tail_release_custom(top2_share=0.25, penalty_start=0.35, penalty_floor=0.25)
    trigger_patch = _patch_weakflow_sectorbias_kr_etf_only(sector_bias_min, trim_fraction)
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Weak Flow Sector Bias KR ETF Sweep",
        "",
        "## Purpose",
        "",
        "- test a narrower non-concentration representative-type trigger",
        "- require both weak-flow and high US IT/Energy bias, then trim top2 into KR ETF recipients",
        "",
        "## Current Read",
        "",
        f"- representative benchmark: `{summary['representative_variant']}`",
        f"- best weak-flow sector-bias point: `{summary['best_variant']}`",
        f"- best CAGR / MDD: `{_pct(summary['best_cagr'])}` / `{_pct(summary['best_mdd'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Sector Bias Min | Trim | Switch Count | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | {_pct(row['SectorBiasMin'])} | {_pct(row['TrimFraction'])} | "
...
def main() -> None:
    ...
    for sector_bias_min, trim_fraction in [
        (0.60, 0.18),
        (0.60, 0.20),
        (0.65, 0.18),
        (0.65, 0.20),
        ...
        variant_name = _compose_variant_name(sector_bias_min, trim_fraction)
        variant = replace(strongest, name=variant_name)
        result, switch_count = _run_with_baseline_switch_carry(
            variant=variant,
            fallback_variant=baseline,
            redistribution_patch=_compose_patch(sector_bias_min, trim_fraction),
    ...
    compare.to_csv(
        OUTPUT_DIR / "weakflow_sectorbias_kr_etf_sweep_compare.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    if best["MDD"] > representative_mdd + MDD_TOL and best["CAGR"] >= representative_cagr - 1e-12:
        summary["verdict"] = (
            f"the weak-flow sector-bias KR ETF trigger finds a representative challenger. `{summary['best_variant']}` reaches "
        ...
        summary["verdict"] = (
            f"the weak-flow sector-bias KR ETF trigger improves drawdown beyond the representative but pays for it in CAGR. "
        ...
        summary["verdict"] = (
            f"the weak-flow sector-bias KR ETF trigger becomes a growth-heavy alternative, not a representative replacement. "
        ...
        summary["verdict"] = (
            f"the weak-flow sector-bias KR ETF trigger improves on the base anchor but does not replace the representative. "
            f"`{summary['best_variant']}` reaches {_pct(summary['best_cagr'])} / {_pct(summary['best_mdd'])}."
        )
    else:
        summary["verdict"] = (
            f"the weak-flow sector-bias KR ETF trigger does not improve the current representative tradeoff. "
            f"`{summary['best_variant']}` does not beat `{representative_variant}` on the benchmark balance."
        )

    (OUTPUT_DIR / "weakflow_sectorbias_kr_etf_sweep_summary.json").write_text(
        json.dumps(summary, indent=2),
        encoding="utf-8",
    )
    (OUTPUT_DIR / "weakflow_sectorbias_kr_etf_sweep_review.md").write_text(
