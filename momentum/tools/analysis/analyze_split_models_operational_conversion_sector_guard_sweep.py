...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_sector_guard_sweep"
...
def _patch_sector_guard(reduction_fraction: float, threshold_gap: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        target_weights = pd.to_numeric(out.loc[target_mask, "TargetWeight"], errors="coerce").fillna(0.0)
        if target_weights.empty or float(target_weights.sum()) <= 0:
            return out

        reduced_weights = target_weights * (1.0 - reduction_fraction)
        released = float(target_weights.sum() - reduced_weights.sum())
        if released <= 0:
            return out

        out.loc[target_mask, "TargetWeight"] = reduced_weights

        recipient_mask = ~target_mask
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Sector Guard Sweep",
        "",
        "## Purpose",
        "",
        "- test a weak-flow sector guard on top of the best redistribution drawdown-control point",
        "- only cut US Information Technology / Energy exposure when the top2 flow profile softens",
        "",
        "## Current Read",
        "",
        f"- redistribution base point: `{summary['base_variant']}`",
        f"- best sector-guard point: `{summary['best_variant']}`",
        f"- best sector-guard MDD: `{_pct(summary['best_mdd'])}`",
        f"- best sector-guard CAGR: `{_pct(summary['best_cagr'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Bucket | Sector Cut | Flow Gap | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | `{row['Bucket']}` | {_pct(row['ReductionFraction'])} | {_pct(row['ThresholdGap'])} | "
...
def main() -> None:
    ...
    for reduction_fraction, threshold_gap in grid:
        variant_name = (
            f"{BASE_VARIANT}_sectorguard_cut{int(round(reduction_fraction * 100)):02d}_gap"
            f"{int(round(threshold_gap * 100)):02d}"
        )
        variant = replace(strongest, name=variant_name)
        result = _run_with_patch(
            variant,
            _compose_patches(base_patch, _patch_sector_guard(reduction_fraction, threshold_gap)),
            universe,
            price_cache,
            flow_cache,
            monthly_close,
        ...
        summary["ReductionFraction"] = reduction_fraction
    ...
    base_summary["ReductionFraction"] = 0.0
    ...
    compare.to_csv(OUTPUT_DIR / "sector_guard_sweep_compare.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "best_reduction_fraction": float(best["ReductionFraction"]),
        "best_threshold_gap": float(best["ThresholdGap"]),
        "ranked_rows": compare.to_dict(orient="records"),
    }
    if summary["best_variant"] == BASE_VARIANT:
        summary["verdict"] = (
            f"the weak-flow sector guard axis fails: no guarded point beats `{BASE_VARIANT}`. "
            f"Cutting US IT / Energy exposure does not buy enough drawdown relief to justify the give-up."
        )
    else:
        summary["verdict"] = (
            f"the weak-flow sector guard finds `{summary['best_variant']}` as the cleanest guarded point; "
    ...
    (OUTPUT_DIR / "sector_guard_sweep_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "sector_guard_sweep_review.md").write_text(_build_markdown(summary), encoding="utf-8")
