...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_weakflow_kr_etf_trigger_sweep"
...
def _compose_variant_name(flow_gap: float, trim_fraction: float) -> str:
    return (
        f"{BASE_VARIANT}_weakflowkretf"
        f"_gap{int(round(flow_gap * 100)):02d}"
        f"_trim{int(round(trim_fraction * 100)):02d}"
        f"_top{CARRY_COUNT}"
    )


def _patch_weakflow_kr_etf_only(flow_gap: float, trim_fraction: float):
    def patch(book: pd.DataFrame) -> pd.DataFrame:
        ...
        if not _weak_flow_trigger(ranked, threshold_gap=flow_gap, sector_bias_min=None):
            return out
...
def _compose_patch(flow_gap: float, trim_fraction: float):
    base_patch = _patch_tail_release_custom(top2_share=0.25, penalty_start=0.35, penalty_floor=0.25)
    weakflow_patch = _patch_weakflow_kr_etf_only(flow_gap, trim_fraction)

    def patch(book: pd.DataFrame) -> pd.DataFrame:
        return weakflow_patch(base_patch(book))
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Weak Flow KR ETF Trigger Sweep",
        "",
        "## Purpose",
        "",
        "- test a non-concentration representative-type trigger family",
        "- use weak-flow detection only, then trim top2 into KR ETF recipients inside the current representative structure",
        "",
        "## Current Read",
        "",
        f"- representative benchmark: `{summary['representative_variant']}`",
        f"- best weak-flow point: `{summary['best_variant']}`",
        f"- best CAGR / MDD: `{_pct(summary['best_cagr'])}` / `{_pct(summary['best_mdd'])}`",
        "",
        "## Ranked Sweep",
        "",
        "| Rank | Variant | Flow Gap | Trim | Switch Count | CAGR | MDD | Sharpe | Neg WF |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_rows"], start=1):
        lines.append(
            f"| {idx} | `{row['Variant']}` | {_pct(row['FlowGap'])} | {_pct(row['TrimFraction'])} | "
...
def main() -> None:
    ...
    for flow_gap, trim_fraction in [
        ...
        variant_name = _compose_variant_name(flow_gap, trim_fraction)
        variant = replace(strongest, name=variant_name)
        result, switch_count = _run_with_baseline_switch_carry(
            variant=variant,
            fallback_variant=baseline,
            redistribution_patch=_compose_patch(flow_gap, trim_fraction),
    ...
    compare.to_csv(
        OUTPUT_DIR / "weakflow_kr_etf_trigger_sweep_compare.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    summary = {
        ...
        "best_flow_gap": None if pd.isna(best["FlowGap"]) else float(best["FlowGap"]),
    ...
    if best["MDD"] > representative_mdd + MDD_TOL and best["CAGR"] >= representative_cagr - 1e-12:
        summary["verdict"] = (
            f"the weak-flow KR ETF trigger finds a representative challenger. `{summary['best_variant']}` reaches "
        ...
        summary["verdict"] = (
            f"the weak-flow KR ETF trigger improves drawdown beyond the representative but pays for it in CAGR. "
        ...
        summary["verdict"] = (
            f"the weak-flow KR ETF trigger becomes a growth-heavy alternative, not a representative replacement. "
        ...
        summary["verdict"] = (
            f"the weak-flow KR ETF trigger improves on the base anchor but does not replace the representative. "
            f"`{summary['best_variant']}` reaches {_pct(summary['best_cagr'])} / {_pct(summary['best_mdd'])}."
        )
    else:
        summary["verdict"] = (
            f"the weak-flow KR ETF trigger does not improve the current representative tradeoff. "
            f"`{summary['best_variant']}` does not beat `{representative_variant}` on the benchmark balance."
        )

    (OUTPUT_DIR / "weakflow_kr_etf_trigger_sweep_summary.json").write_text(
        json.dumps(summary, indent=2),
        encoding="utf-8",
    )
    (OUTPUT_DIR / "weakflow_kr_etf_trigger_sweep_review.md").write_text(
