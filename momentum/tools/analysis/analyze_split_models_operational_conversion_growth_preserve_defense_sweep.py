...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_growth_preserve_defense_sweep"
TRIM22_FRACTION = 0.22
TRIM22_GAP = 0.02
TRIM22_CARRY_COUNT = 2
GROWTH_PRESERVE_SYMBOLS = {"NVDA", "PLTR"}
...
def _growth_preserve_trigger(book: pd.DataFrame, min_weight: float, min_avg_flow: float | None) -> bool:
    if book.empty or "TargetWeight" not in book.columns:
        return False
    target = book[book["Symbol"].astype(str).isin(GROWTH_PRESERVE_SYMBOLS)].copy()
    if target.empty:
        return False
    target["TargetWeight"] = pd.to_numeric(target["TargetWeight"], errors="coerce").fillna(0.0)
    if float(target["TargetWeight"].sum()) < min_weight:
        return False
    if min_avg_flow is not None:
        target["FlowScore"] = pd.to_numeric(target["FlowScore"], errors="coerce").fillna(0.0)
        if float(target["FlowScore"].mean()) < min_avg_flow:
            return False
    return True


def _run_growth_preserve_defense(
    *,
    variant: TradingVariant,
    fallback_variant: TradingVariant,
    preserve_min_weight: float,
    preserve_min_avg_flow: float | None,
    preserve_trim_fraction: float,
    ...
    full_defense_patch = _symbol_trim_patch(1.0, EXTENDED_DRAG_SYMBOLS)
    preserve_patch = _symbol_trim_patch(preserve_trim_fraction, EXTENDED_DRAG_SYMBOLS)
    weak_switch_count = 0
    ...
    def wrapped_build(metrics, flow_snapshot, cfg_inner, variant=None, prev_hold_keys=None, **kwargs):
        ...
        if (
            ...
            if _growth_preserve_trigger(current_book, preserve_min_weight, preserve_min_avg_flow):
                preserve_dates.append(asof)
                return preserve_patch(current_book)
            return full_defense_patch(current_book)
        return current_book
    ...
    try:
        ...
        diagnostics = {
            ...
            "preserve_count": len(preserve_dates),
...
def _run_candidate(
    *,
    ...
    flow_tag = "noflow" if spec["preserve_min_avg_flow"] is None else f"flow{int(round(float(spec['preserve_min_avg_flow']) * 100)):02d}"
    variant_name = (
        f"growth_preserve_w{int(round(float(spec['preserve_min_weight']) * 100)):02d}"
        f"_{flow_tag}_trim{int(round(float(spec['preserve_trim_fraction']) * 100)):02d}"
    )
    result, diagnostics = _run_growth_preserve_defense(
        variant=replace(strongest, name=variant_name),
        fallback_variant=baseline,
        preserve_min_weight=float(spec["preserve_min_weight"]),
        preserve_min_avg_flow=None if spec["preserve_min_avg_flow"] is None else float(spec["preserve_min_avg_flow"]),
        preserve_trim_fraction=float(spec["preserve_trim_fraction"]),
        universe=universe,
    ...
    row.update(
        {
            "PreserveMinWeight": float(spec["preserve_min_weight"]),
            "PreserveMinAvgFlow": spec["preserve_min_avg_flow"],
            "PreserveTrimFraction": float(spec["preserve_trim_fraction"]),
            "DefenseCount": int(diagnostics["defense_count"]),
            "PreserveCount": int(diagnostics["preserve_count"]),
            ...
            "PreserveDates": diagnostics["preserve_dates"],
        }
    )
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Growth-Preserve Defense Sweep",
        "",
        ...
        "## Ranked Specs",
        "",
        "| Rank | Weight | Flow | Preserve Trim | Pass | Passed Shifts | Worst CAGR | Worst MDD | Worst Neg WF |",
        "| ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: |",
    ]
    for idx, row in enumerate(summary["ranked_specs"], start=1):
        flow = "none" if row["preserve_min_avg_flow"] is None else _pct(row["preserve_min_avg_flow"])
        lines.append(
            f"| {idx} | {_pct(row['preserve_min_weight'])} | {flow} | {_pct(row['preserve_trim_fraction'])} | "
...
def main() -> None:
    ...
    specs = [
        {"preserve_min_weight": 0.30, "preserve_min_avg_flow": None, "preserve_trim_fraction": 0.0},
        {"preserve_min_weight": 0.30, "preserve_min_avg_flow": 0.20, "preserve_trim_fraction": 0.0},
        {"preserve_min_weight": 0.50, "preserve_min_avg_flow": None, "preserve_trim_fraction": 0.0},
        {"preserve_min_weight": 0.30, "preserve_min_avg_flow": None, "preserve_trim_fraction": 0.5},
    ]
    ...
    summary = {
        ...
        "sweep_decision": "PASS_GROWTH_PRESERVE_DEFENSE_FOUND" if best["all_shifts_pass"] else "BLOCK_GROWTH_PRESERVE_DEFENSE",
    ...
    pd.DataFrame(all_rows).drop(columns=["DefenseDates", "PreserveDates"], errors="ignore").to_csv(
        OUTPUT_DIR / "growth_preserve_defense_sweep_rows.csv", index=False, encoding="utf-8-sig"
    )
    (OUTPUT_DIR / "growth_preserve_defense_sweep_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "growth_preserve_defense_sweep.md").write_text(_build_markdown(summary), encoding="utf-8")
