...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_start_shift_repair_sweep"


def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Start-Shift Repair Sweep",
        "",
        f"Generated: `{summary['generated_at']}`",
        "",
        "## Best Candidate",
        "",
        f"- decision: `{summary['repair_decision']}`",
...
def main() -> None:
    ...
    specs = [
        {"min_symbol_weight": 0.09, "min_sector_weight": 0.28, "max_defense_count": 5, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.09, "min_sector_weight": 0.28, "max_defense_count": 6, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.09, "min_sector_weight": 0.28, "max_defense_count": 7, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.10, "min_sector_weight": 0.30, "max_defense_count": 7, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.11, "min_sector_weight": 0.32, "max_defense_count": 7, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.11, "min_sector_weight": 0.32, "max_defense_count": 8, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.12, "min_sector_weight": 0.35, "max_defense_count": 7, "trim_fraction": 1.0},
        {"min_symbol_weight": 0.12, "min_sector_weight": 0.35, "max_defense_count": 8, "trim_fraction": 1.0},
    ]
    ...
    summary = {
        ...
        "repair_decision": "PASS_START_SHIFT_REPAIR_FOUND" if best["all_shifts_pass"] else "BLOCK_NO_START_SHIFT_REPAIR",
    ...
    pd.DataFrame(all_rows).drop(columns=["DefenseDates"], errors="ignore").to_csv(
        OUTPUT_DIR / "start_shift_repair_sweep_rows.csv", index=False, encoding="utf-8-sig"
    )
    (OUTPUT_DIR / "start_shift_repair_sweep_summary.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
    (OUTPUT_DIR / "start_shift_repair_sweep.md").write_text(_build_markdown(summary), encoding="utf-8")
