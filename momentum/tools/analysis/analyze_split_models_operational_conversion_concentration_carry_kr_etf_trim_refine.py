...
ROOT = REPO_ROOT
OUTPUT_DIR = ROOT / "output" / "split_models_operational_conversion_concentration_carry_kr_etf_trim_refine"
MDD_TOL = 1e-9
...
def _compose_variant_name(trim_fraction: float) -> str:
    return (
        f"{BASE_VARIANT}_conccarrykretf_ct{int(round(CONCENTRATION_THRESHOLD * 100)):02d}"
...
def _build_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# Split Models Operational Conversion Concentration Carry KR ETF Trim Refine",
        "",
        "## Purpose",
        "",
        "- keep the best KR ETF only recipient structure fixed",
        "- narrow only the trim strength to see whether the new drawdown gain can be preserved with less CAGR damage",
        "",
        "## Current Read",
        "",
...
def main() -> None:
    ...
    for trim_fraction in [0.14, 0.16, 0.18, 0.20, 0.22]:
    ...
    compare.to_csv(OUTPUT_DIR / "concentration_carry_kr_etf_trim_refine_compare.csv", index=False, encoding="utf-8-sig")
    ...
    if best["MDD"] > base_summary["MDD"] + MDD_TOL:
        summary["verdict"] = (
            f"the KR ETF trim refine improves drawdown: `{summary['best_variant']}` lifts MDD to "
        ...
        summary["verdict"] = (
            f"the KR ETF trim refine improves quality but not drawdown. `{summary['best_variant']}` keeps "
            f"MDD flat at {_pct(summary['best_mdd'])} while moving CAGR to {_pct(summary['best_cagr'])}."
        )
    else:
        summary["verdict"] = (
            f"the KR ETF trim refine fails: no trim point improves drawdown or quality versus `{BASE_VARIANT}`."
        )

    (OUTPUT_DIR / "concentration_carry_kr_etf_trim_refine_summary.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
    (OUTPUT_DIR / "concentration_carry_kr_etf_trim_refine_review.md").write_text(
