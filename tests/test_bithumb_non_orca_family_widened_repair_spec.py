...
MODULE_PATH = Path(r"C:\AI\build_bithumb_non_orca_family_widened_repair_spec.py")
SPEC = importlib.util.spec_from_file_location("build_bithumb_non_orca_family_widened_repair_spec", MODULE_PATH)
...
class BithumbNonOrcaFamilyWidenedRepairSpecTests(unittest.TestCase):
    def test_builds_widened_repair_spec_without_order_paths(self) -> None:
        stop = {
            "status": "NON_ORCA_PASS_FOLD_REPAIR_STOP_CONDITION_READY",
            "recommended_branch_action": "STOP_NON_ORCA_PASS_FOLD_REPAIR_GRID",
            "recommended_next_research_action": "WIDEN_NON_ORCA_ENTRY_FAMILY_OR_DATA_WINDOW_REPAIR",
        ...
        self.assertEqual(report["repair_targets"][0]["parameter_grid"]["lookback_bars"], [2, 3, 5])
        self.assertEqual(report["repair_targets"][0]["parameter_grid"]["volume_window"], [10, 20, 40])
        self.assertEqual(report["repair_targets"][0]["parameter_grid"]["volume_ratio_floor"], [0.8, 1.0, 1.2])
        self.assertEqual(report["repair_targets"][0]["parameter_grid"]["momentum_threshold"], [0.01, 0.02, 0.035])
