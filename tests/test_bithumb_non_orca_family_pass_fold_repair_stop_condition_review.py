...
MODULE_PATH = Path(r"C:\AI\build_bithumb_non_orca_family_pass_fold_repair_stop_condition_review.py")
SPEC = importlib.util.spec_from_file_location(
    "build_bithumb_non_orca_family_pass_fold_repair_stop_condition_review",
    MODULE_PATH,
)
...
class BithumbNonOrcaFamilyPassFoldRepairStopConditionReviewTests(unittest.TestCase):
    def test_ready_when_full_sweep_has_no_oos_or_robustness_pass(self) -> None:
        spec = {
        ...
        sweep = {
            "status": "NON_ORCA_PASS_FOLD_REPAIR_SWEEP_ITERATE",
            "experiment_id": "non_orca",
        ...
        self.assertEqual(report["status"], "NON_ORCA_PASS_FOLD_REPAIR_STOP_CONDITION_READY")
        self.assertTrue(report["ready_for_gatekeeper_review"])
        self.assertEqual(report["recommended_branch_action"], "STOP_NON_ORCA_PASS_FOLD_REPAIR_GRID")
        self.assertEqual(report["recommended_next_research_action"], "WIDEN_NON_ORCA_ENTRY_FAMILY_OR_DATA_WINDOW_REPAIR")
    ...
    def test_blocks_unsafe_sweep(self) -> None:
        spec = {
            "status": "READY_FOR_RESEARCH_SPEC_REVIEW",
            "counts_as_paper_or_live_evidence": False,
            "no_order_assertions": dict(stop.SAFE_ASSERTIONS),
        ...
        self.assertEqual(report["status"], "NON_ORCA_PASS_FOLD_REPAIR_STOP_CONDITION_BLOCKED")
