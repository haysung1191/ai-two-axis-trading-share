...
MODULE_PATH = Path(r"C:\AI\build_bithumb_non_orca_family_pass_fold_repair_sweep.py")
SPEC = importlib.util.spec_from_file_location("build_bithumb_non_orca_family_pass_fold_repair_sweep", MODULE_PATH)
...
class BithumbNonOrcaFamilyPassFoldRepairSweepTests(unittest.TestCase):
    def source_spec(self) -> dict:
        return {
            "status": "READY_FOR_RESEARCH_SPEC_REVIEW",
            "experiment_id": "non_orca_repair",
    ...
    def test_sweep_reports_passing_child_without_order_permissions(self) -> None:
        with (
            ...
            patch.object(
                ...
                return_value={
                    "candidate_id": "pola_passfoldrepair_001",
        ...
        self.assertEqual(report["status"], "NON_ORCA_PASS_FOLD_REPAIR_SWEEP_PASS")
    ...
    def test_sweep_iterates_without_robustness_pass(self) -> None:
        with (
            patch.object(sweep.repair.sweep.backtest, "fetch_candles", return_value=[{"close": 1.0}]),
            patch.object(
                sweep.repair,
        ...
        self.assertEqual(report["status"], "NON_ORCA_PASS_FOLD_REPAIR_SWEEP_ITERATE")
        self.assertIn("NO_NON_ORCA_REPAIR_ROBUSTNESS_PASS", report["blockers"])
