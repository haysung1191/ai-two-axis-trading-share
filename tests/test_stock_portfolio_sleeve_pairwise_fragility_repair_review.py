...
MODULE_PATH = Path(r"C:\AI\build_stock_portfolio_sleeve_pairwise_fragility_repair_review.py")
SPEC = importlib.util.spec_from_file_location(
    "build_stock_portfolio_sleeve_pairwise_fragility_repair_review", MODULE_PATH
)
...
class StockPortfolioSleevePairwiseFragilityRepairReviewTests(unittest.TestCase):
    ...
    def pairwise_packet(self) -> dict:
        return {
            ...
            "scenarios": [
                {
                    "scenario_id": "drop_both_stocks",
                    "removed_candidate_ids": ["candidate_3", "candidate_4"],
                    "removed_lanes": ["kis_stocks", "kis_stocks"],
                    "remaining_candidate_ids": ["candidate_0", "candidate_1", "candidate_2"],
                    "fragile": True,
                    "fragility_reasons": ["LANE_WEIGHT_ABOVE_80PCT"],
                },
                {
                    "scenario_id": "drop_mixed",
                    "removed_candidate_ids": ["candidate_0", "candidate_3"],
                    "removed_lanes": ["kis_etfs", "kis_stocks"],
                    "remaining_candidate_ids": ["candidate_1", "candidate_2", "candidate_4"],
                    "fragile": False,
                    "fragility_reasons": [],
                },
            ],
    ...
    def test_repairs_lane_concentration_with_cash_buffer_without_order_paths(self) -> None:
        report = review.build_report(self.sleeve_packet(), self.pairwise_packet())

        self.assertEqual(report["status"], "PORTFOLIO_SLEEVE_PAIRWISE_FRAGILITY_REPAIR_REVIEW_READY")
    ...
    def test_blocks_unsafe_source_order_path(self) -> None:
        pairwise = self.pairwise_packet()
        pairwise["no_order_assertions"]["broker_submit_allowed_by_this_report"] = True

        report = review.build_report(self.sleeve_packet(), pairwise)

        self.assertEqual(report["status"], "PORTFOLIO_SLEEVE_PAIRWISE_FRAGILITY_REPAIR_REVIEW_BLOCKED")
