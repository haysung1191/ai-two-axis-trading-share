...
import scripts.compare_btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_entry as target
...
def _base_release_review_report() -> dict:
    return {
        "stack_context": {
...
def test_pullthrough_live_shadow_locked_release_entry_marks_ready(
    tmp_path: Path,
) -> None:
    ...
    assert requirements["challenger_live_shadow_locked_candidate_release_review_ready"] is True
    assert requirements["operating_index_live_shadow_locked_candidate_release_review_ready"] is True
    assert requirements["operating_brief_live_shadow_locked_candidate_release_review_ready"] is True
    assert requirements["dashboard_live_shadow_locked_candidate_release_review_ready"] is True
    assert requirements["queue_lane_mirrored"] is True
    assert verdict["challenger_live_shadow_locked_release_entry_ready"] is True


def test_pullthrough_live_shadow_locked_release_entry_points_to_locked_release_candidate_review(
    tmp_path: Path,
) -> None:
    analysis_dir = tmp_path / "analysis_results"
    _write_json(
    ...
    assert (
        verdict["challenger_live_shadow_locked_release_entry_lane"]
        == "challenger_live_shadow_locked_release_queue"
    )
    assert verdict["next_step_now"] == "challenger_live_shadow_locked_release_candidate_review"
