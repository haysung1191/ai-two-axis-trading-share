...
import scripts.compare_btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_governance_check as target
...
def _base_candidate_review_report() -> dict:
    return {
        "stack_context": {
...
def test_pullthrough_live_shadow_locked_release_governance_check_marks_ready(
    tmp_path: Path,
) -> None:
    ...
    assert requirements["challenger_live_shadow_locked_release_candidate_review_ready"] is True
    assert requirements["operating_index_live_shadow_locked_release_candidate_review_ready"] is True
    assert requirements["operating_brief_live_shadow_locked_release_candidate_review_ready"] is True
    assert requirements["dashboard_live_shadow_locked_release_candidate_review_ready"] is True
    assert requirements["queue_lane_mirrored"] is True
    assert verdict["challenger_live_shadow_locked_release_governance_check_ready"] is True


def test_pullthrough_live_shadow_locked_release_governance_check_points_to_entry(
    tmp_path: Path,
) -> None:
    analysis_dir = tmp_path / "analysis_results"
    _write_json(
    ...
    assert (
        verdict["challenger_live_shadow_locked_release_governance_check_lane"]
        == "challenger_live_shadow_locked_release_governance_check_queue"
    )
    assert verdict["next_step_now"] == "challenger_live_shadow_locked_release_governance_entry"
