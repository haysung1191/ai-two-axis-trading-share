...
from strategies.btc_4h_trend_regime_persistence_mvp import (
    compute_btc_4h_trend_regime_persistence_signals,
)
...
def test_btc_4h_parent_strategy_enters_on_trend_persistence_without_strength_gate() -> None:
    frame = _build_frame([100.0 + (i * 0.2) for i in range(120)])

    signal = compute_btc_4h_trend_regime_persistence_signals(frame)
...
def test_btc_4h_parent_strategy_exits_on_regime_breakdown() -> None:
...
def test_btc_4h_parent_strategy_waits_for_confirmed_fast_break_before_exit() -> None:
    rising = [100.0 + (i * 1.5) for i in range(120)]
    tail = [rising[-1] - 25.0, rising[-1] + 5.0, rising[-1] - 30.0, rising[-1] - 40.0]
    frame = _build_frame(rising + tail)

    signal = compute_btc_4h_trend_regime_persistence_signals(
        frame,
...
def test_btc_4h_parent_strategy_returns_flat_signal_when_history_is_short() -> None:
