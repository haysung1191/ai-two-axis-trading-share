...
class Btc1dDefensiveCashFilterWalkForwardConfig:
    symbol: str = "BTCUSDT"
    interval: str = "1d"
    periods: int = 2200
    fee_bps: float = 8.0
    ...
    candidate_label: str = "btc_1d_defensive_cash_filter"
    floor_ema_window: int = 96
    rebound_window: int = 3
    rebound_threshold: float = 0.01
    dip_window: int = 8
    dip_threshold: float = -0.035
    volatility_window: int = 12
    max_volatility: float = 0.03


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTC 1d walk-forward diagnostic for the defensive cash filter strategy.")
...
def parse_args(argv: list[str] | None = None) -> Btc1dDefensiveCashFilterWalkForwardConfig:
    args = build_parser().parse_args(argv)
    return Btc1dDefensiveCashFilterWalkForwardConfig(
        symbol=args.symbol,
        ...
        candidate_label="btc_1d_defensive_cash_filter",
...
def load_ohlcv(config: Btc1dDefensiveCashFilterWalkForwardConfig):
    import pandas as pd
...
def build_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# BTC 1d Defensive Cash Filter Walk-Forward Diagnostic",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    strategy = load_strategy("major_4h_defensive_cash_filter", strategies_dir=Path("strategies"))
    ohlcv = load_ohlcv(config)
    parameters = {
        "floor_ema_window": config.floor_ema_window,
        "rebound_window": config.rebound_window,
        "rebound_threshold": config.rebound_threshold,
        "dip_window": config.dip_window,
    ...
    payload = {
        "run_id": f"btc-1d-defensive-cash-filter-walkdiag-{datetime.now(tz=UTC).strftime('%Y%m%dT%H%M%SZ')}",
    ...
    analysis_json = Path("analysis_results") / f"btc_1d_defensive_cash_filter_walk_forward_diagnostic_{stamp}.json"
    analysis_md = Path("analysis_results") / f"btc_1d_defensive_cash_filter_walk_forward_diagnostic_{stamp}.md"
