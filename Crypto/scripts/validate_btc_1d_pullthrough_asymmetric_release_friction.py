...
from scripts.validate_btc_1d_pullthrough_asymmetric_release_candidate import parse_args as parse_candidate_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d pullthrough asymmetric release candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d pullthrough asymmetric release candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "pullthrough asymmetric release candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "pullthrough asymmetric release candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "pullthrough asymmetric release candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "pullthrough asymmetric release candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Pullthrough Asymmetric Release Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "candidate": "pullthrough_asymmetric_release_tighter_exit",
    ...
    json_path = args.analysis_dir / f"btc_1d_pullthrough_asymmetric_release_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_pullthrough_asymmetric_release_friction_{stamp}.md"
