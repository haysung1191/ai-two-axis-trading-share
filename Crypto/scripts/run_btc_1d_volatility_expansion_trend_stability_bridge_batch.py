...
from app.domains.experiments.btc_1d_volatility_expansion_trend_stability_bridge_batch import (
    Btc1dVolatilityExpansionTrendStabilityBridgeBatchService,
    Btc1dVolatilityExpansionTrendStabilityBridgeConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d volatility expansion trend stability bridge batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = Btc1dVolatilityExpansionTrendStabilityBridgeBatchService(analysis_results_dir=args.analysis_dir).run_batch(
        config
    )
