...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    release_candidate_review = _load_json(
        analysis_dir
    ...
    context = dict(release_candidate_review.get("stack_context", {}) or {})
    candidate = dict(release_candidate_review.get("candidate_profile", {}) or {})
    requirements = dict(
        release_candidate_review.get(
    ...
    governance_requirements = {
        "challenger_live_shadow_locked_release_candidate_review_ready": bool(
            verdict.get(
        ...
        "operating_index_live_shadow_locked_release_candidate_review_ready": bool(
            operating_index.get(
                "attack_challenger_live_shadow_locked_release_candidate_review_ready",
                False,
            )
        ),
        "operating_brief_live_shadow_locked_release_candidate_review_ready": bool(
            operating_brief.get(
                "attack_challenger_live_shadow_locked_release_candidate_review_ready",
                False,
            )
        ),
        "dashboard_live_shadow_locked_release_candidate_review_ready": bool(
    ...
    governance_lane = (
        ...
        if governance_ready
        else "challenger_live_shadow_locked_release_governance_check_repair_hold"
    )
    next_step_now = (
        "challenger_live_shadow_locked_release_governance_entry"
        if governance_ready
        else "repair_challenger_live_shadow_locked_release_governance_check"
    )
    ...
    return {
        ...
        "challenger_live_shadow_locked_release_governance_check_requirements": governance_requirements,
        "challenger_live_shadow_locked_release_governance_check_verdict": {
            "challenger_live_shadow_locked_release_governance_check_ready": governance_ready,
            "challenger_live_shadow_locked_release_governance_check_lane": governance_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Locked release candidate review is green and every operator-facing mirror agrees, so the challenger can enter locked release governance check."
                if governance_ready
                else "The challenger cleared locked release candidate review, but the governance-check mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger locked-release governance-check candidate.",
            (
                "Advance to `challenger_live_shadow_locked_release_governance_entry` because locked release candidate review and operator-facing mirrors are all green."
                if governance_ready
                else "Do not advance into locked release governance check until all governance-check requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes locked release governance check.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    verdict = report["challenger_live_shadow_locked_release_governance_check_verdict"]
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Shadow Locked Release Governance Check",
            "",
            ...
            f"- Challenger live shadow locked release governance check ready: `{verdict['challenger_live_shadow_locked_release_governance_check_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_shadow_locked_release_governance_check_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_governance_check_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_governance_check_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
