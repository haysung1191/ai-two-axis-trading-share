...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTCUSDT 1d paper validation for the volatility expansion reclaim ratio-112 candidate.")
...
def parse_args(argv: list[str] | None = None) -> BtcPaperValidationConfig:
    ...
    return BtcPaperValidationConfig(
        ...
        strategy_name="btc_1d_volatility_expansion_reclaim_sr112",
        strategy_category="volatility_breakout",
        hypothesis="BTCUSDT 1d volatility expansion reclaim ratio-112 candidate targets 40%+ CAGR with sub-20% drawdown while reducing dependence on the exact ATR expansion threshold.",
        ema_fast_window=20,
        ema_slow_window=50,
        atr_window=14,
        atr_multiple=3.5,
