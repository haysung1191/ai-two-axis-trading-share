...
from app.domains.experiments.btc_1d_upside_imbalance_reclaim_continuation_high_cagr_batch import (
    ...
    Btc1dUpsideImbalanceReclaimContinuationHighCagrConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d upside imbalance reclaim continuation high-CAGR batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dUpsideImbalanceReclaimContinuationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dUpsideImbalanceReclaimContinuationHighCagrBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
