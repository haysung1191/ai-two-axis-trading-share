...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTCUSDT 1d low-volatility cap batch.")
...
def parse_args(argv: list[str] | None = None) -> Btc1dLowVolatilityCapConfig:
    args = build_parser().parse_args(argv)
    return Btc1dLowVolatilityCapConfig(
        symbol=args.symbol,
...
def main(argv: list[str] | None = None) -> int:
    result = Btc1dLowVolatilityCapBatchService().run_batch(parse_args(argv))
