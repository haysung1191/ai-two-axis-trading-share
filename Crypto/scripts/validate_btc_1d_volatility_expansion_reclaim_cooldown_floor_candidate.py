...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run BTCUSDT 1d paper validation for the volatility expansion reclaim cooldown floor candidate."
    )
...
def parse_args(argv: list[str] | None = None) -> BtcPaperValidationConfig:
    ...
    return BtcPaperValidationConfig(
        ...
        strategy_name="btc_1d_volatility_expansion_reclaim_cdf12s",
        strategy_category="volatility_breakout",
        hypothesis="BTCUSDT 1d volatility expansion reclaim cooldown floor candidate targets long-span survivability with drawdown held below 20% while preserving as much CAGR as possible.",
        ema_fast_window=20,
        ema_slow_window=50,
        atr_window=14,
        atr_multiple=3.5,
