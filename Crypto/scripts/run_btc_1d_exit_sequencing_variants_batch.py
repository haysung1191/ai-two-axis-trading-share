...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTCUSDT 1d exit sequencing variants batch.")
...
def parse_args(argv: list[str] | None = None) -> Btc1dExitSequencingVariantsConfig:
    args = build_parser().parse_args(argv)
    return Btc1dExitSequencingVariantsConfig(
        symbol=args.symbol,
...
def main(argv: list[str] | None = None) -> int:
    result = Btc1dExitSequencingVariantsBatchService().run_batch(parse_args(argv))
