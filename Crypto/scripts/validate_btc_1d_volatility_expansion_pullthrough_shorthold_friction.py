...
from scripts.validate_btc_1d_volatility_expansion_pullthrough_shorthold_candidate import parse_args as parse_candidate_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d volatility expansion pullthrough softer-setup hold-31 candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d volatility expansion pullthrough softer-setup hold-31 candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "volatility expansion pullthrough softer-setup hold-31 candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "volatility expansion pullthrough softer-setup hold-31 candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "volatility expansion pullthrough softer-setup hold-31 candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "volatility expansion pullthrough softer-setup hold-31 candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Volatility Expansion Pullthrough Softer Setup Hold31 Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = args.analysis_dir / f"btc_1d_volatility_expansion_pullthrough_shorthold_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_volatility_expansion_pullthrough_shorthold_friction_{stamp}.md"
