...
from app.domains.experiments.btc_1d_volatility_expansion_trend_sensitivity_relief_batch import (
    Btc1dVolatilityExpansionTrendSensitivityReliefBatchService,
    Btc1dVolatilityExpansionTrendSensitivityReliefConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d volatility expansion trend sensitivity relief batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = Btc1dVolatilityExpansionTrendSensitivityReliefBatchService(analysis_results_dir=args.analysis_dir).run_batch(
        config
    )
