...
WALK_FORWARD_CANDIDATE = "volatility_expansion_pullthrough_softer_setup_hold31"
VALIDATION_STRATEGY = "btc_1d_volatility_expansion_pullthrough_v3"
...
def _pick_validation(analysis_dir: Path) -> tuple[Path, dict]:
    path = _latest_json(
        analysis_dir,
        f"{VALIDATION_STRATEGY}_btcusdt_1d_2200_paper_validation_*.json",
    )
...
def _pick_friction(analysis_dir: Path) -> tuple[Path, dict]:
    candidates = sorted(
        analysis_dir.glob("btc_1d_volatility_expansion_pullthrough_shorthold_friction_*.json"),
    ...
    for path in candidates:
        ...
        if candidate == WALK_FORWARD_CANDIDATE:
            return path, payload
    raise FileNotFoundError("No pullthrough shorthold friction artifact found for softer_setup_hold31 candidate.")
...
def _pick_walk_forward(analysis_dir: Path) -> tuple[Path, dict]:
    ...
    for path in candidates:
        ...
        if candidate == WALK_FORWARD_CANDIDATE:
            return path, payload
    raise FileNotFoundError("No walk-forward diagnostic artifact found for softer_setup_hold31 candidate.")
...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    try:
        ...
        return {
            ...
            "candidate_profile": {
                ...
                "paper_validation_sharpe": 0.0,
                "paper_validation_cagr": 0.0,
                "paper_validation_max_drawdown": 0.0,
                "friction_final_decision": "missing_artifact",
                "walk_forward_passed": False,
                "walk_forward_sensitivity_max_drift": 1.0,
                "negative_walk_forward_windows": [],
                "idle_walk_forward_windows": [],
            },
            "artifact_paths": {
                "paper_validation_json": "",
                "friction_json": "",
                "walk_forward_json": "",
            },
            "pullthrough_candidate_stage_review_requirements": {
                "paper_validation_passed": False,
                "friction_stays_green": False,
                "walk_forward_passed": False,
                "walk_forward_no_negative_window": False,
                "sensitivity_drift_within_guardrail": False,
                "unstable_parameters_clear": False,
            },
            "pullthrough_candidate_stage_review_verdict": {
                "candidate_stage_ready": False,
                "candidate_stage_lane": "pullthrough_candidate_stage_unavailable",
                "next_step_now": "continue_fresh_seed_priority_without_pullthrough_artifact",
                "reason": (
                    "The pullthrough shorthold friction artifact is missing, so this lane is unavailable. "
                    "Do not let the stale pullthrough dependency block fresh-seed queue construction."
                ),
            },
            "decision_summary": [
                f"Mark `{WALK_FORWARD_CANDIDATE}` unavailable because `{reason}`.",
                "Continue fresh-seed priority construction from the broader attack conversion board.",
            ],
            "missing_artifact_blocker": reason,
        }
    friction = dict((friction_payload.get("report", {}) if "report" in friction_payload else friction_payload) or {})
    validation_path: Path | None = None
    validation: dict | None = None
    for level in list(friction.get("levels", []) or []):
        if abs(float(level.get("cost_bps", -1.0)) - 8.0) < 1e-9:
            candidate_path = Path(str(level.get("analysis_result_json", "")))
            if candidate_path.exists():
                validation_path = candidate_path
                validation = _load_json(candidate_path)
            break
    if validation is None or validation_path is None:
    ...
    requirements = {
        ...
        "sensitivity_drift_within_guardrail": float(walk_overfit.get("sensitivity_max_drift", 1.0)) <= 0.15,
        "unstable_parameters_clear": len(list(walk_overfit.get("unstable_parameters", []) or [])) == 0,
    }

    review_ready = all(requirements.values())
    queue_lane = (
        "pullthrough_candidate_stage_promotion_queue"
        if review_ready
        else "pullthrough_candidate_stage_repair_hold"
    )
    next_step_now = (
    ...
    reason = (
        "Validation, friction, and walk-forward are all aligned, so the candidate can advance into the attack-comparison lane."
        if review_ready
        else "Validation and friction are green, but walk-forward still contains a negative window or uneven deployment profile, so promotion should stay on hold."
    )
    ...
    return {
        ...
        "pullthrough_candidate_stage_review_requirements": requirements,
        "pullthrough_candidate_stage_review_verdict": {
        ...
        "decision_summary": [
            f"Keep `{WALK_FORWARD_CANDIDATE}` as the active pullthrough candidate-stage reference.",
            (
                "Advance this candidate into attack comparison because validation, friction, and walk-forward all stayed green."
                if review_ready
                else "Do not promote yet; repair the negative walk-forward window before comparing this candidate against the attack main lane."
            ),
...
def _render_markdown(report: dict) -> str:
    ...
    requirements = report["pullthrough_candidate_stage_review_requirements"]
    verdict = report["pullthrough_candidate_stage_review_verdict"]
    paths = report["artifact_paths"]
    return "\n".join(
        [
            "# BTC 1d Pullthrough Candidate Stage Review",
            "",
...
def main() -> int:
    ...
    json_path = ANALYSIS_DIR / f"btc_1d_pullthrough_candidate_stage_review_{stamp}.json"
    md_path = ANALYSIS_DIR / f"btc_1d_pullthrough_candidate_stage_review_{stamp}.md"
    latest_json = ANALYSIS_DIR / "btc_1d_pullthrough_candidate_stage_review_latest.json"
    latest_md = ANALYSIS_DIR / "btc_1d_pullthrough_candidate_stage_review_md_latest.md"
