...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTCUSDT 1d regime-gate variants batch.")
...
def parse_args(argv: list[str] | None = None) -> Btc1dRegimeGateVariantsConfig:
    args = build_parser().parse_args(argv)
    return Btc1dRegimeGateVariantsConfig(
        symbol=args.symbol,
...
def main(argv: list[str] | None = None) -> int:
    result = Btc1dRegimeGateVariantsBatchService().run_batch(parse_args(argv))
