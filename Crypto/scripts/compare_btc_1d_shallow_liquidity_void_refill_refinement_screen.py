...
def _latest_batch_path(analysis_dir: Path) -> Path:
    matches = sorted(
        analysis_dir.glob("btc_1d_shallow_liquidity_void_refill_refinement_batch_*.json"),
        key=lambda path: path.name,
    )
    if not matches:
        raise FileNotFoundError("No shallow liquidity void refill refinement batch artifact found.")
    return matches[-1]
...
def build_report_from_batch(batch: dict) -> dict:
    ...
    return {
        ...
        "refinement_verdict": {
            ...
            "reason": (
                "A refinement mutation compressed sensitivity enough to justify moving this lane into friction retest."
                if ready
                else "Even the best refinement mutation does not compress sensitivity enough yet, so candidate repair search must continue."
            ),
        },
        "decision_summary": [
            (
                f"Best refinement mutation is `{best['variant_label']}` with "
                f"{len(best.get('overfitting_flags', []))} overfitting flags and drift `{float(best.get('sensitivity_max_drift', 0.0)):.4f}`."
            ),
            "Advance only if the refinement result keeps zero overfitting flags and pushes sensitivity drift to 0.18 or below while preserving the attack profile.",
        ],
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare shallow liquidity void refill refinement batch results.")
    ...
    json_path = args.analysis_dir / f"btc_1d_shallow_liquidity_void_refill_refinement_screen_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_shallow_liquidity_void_refill_refinement_screen_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    md_lines = [
        "# BTC 1d Shallow Liquidity Void Refill Refinement Screen",
        "",
        ...
        f"- Has friction-ready seed: `{report['refinement_verdict']['has_friction_ready_seed']}`",
        f"- Next step: `{report['refinement_verdict']['next_step']}`",
        f"- Reason: {report['refinement_verdict']['reason']}",
        "",
    ]
