...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTC 1d shallow liquidity void refill micro-refinement batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dShallowLiquidityVoidRefillMicroRefinementConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dShallowLiquidityVoidRefillMicroRefinementBatchService(
