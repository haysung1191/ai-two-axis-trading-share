...
from app.domains.experiments.btc_paper_validation import BtcPaperValidationConfig
from scripts.validate_btc_1d_volatility_expansion_reclaim_sr112_candidate import parse_args as parse_sr112_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d volatility expansion reclaim defensive candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        strategy_name="btc_1d_volatility_expansion_reclaim_stab_atrs",
        strategy_category=base.strategy_category,
        hypothesis="BTCUSDT 1d volatility expansion reclaim defensive candidate under friction sanity check.",
        ...
        artifact_label=f"{str(base.symbol).lower()}_{str(base.interval).lower()}_{int(base.periods)}_{int(cost_bps)}bps",
        extra_parameters={
            "trend_ema_window": 72,
            "breakout_window": 18,
            "atr_window": 11,
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "defensive candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "defensive candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "defensive candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "defensive candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Volatility Expansion Reclaim Defensive Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = args.analysis_dir / f"btc_1d_volatility_expansion_reclaim_stab_atrs_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_volatility_expansion_reclaim_stab_atrs_friction_{stamp}.md"
