...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d shallow pullback impulse continuation high-CAGR batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dShallowPullbackImpulseContinuationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dShallowPullbackImpulseContinuationHighCagrBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
