...
from app.domains.experiments.btc_1d_one_bar_failed_dip_continuation_high_cagr_batch import (
    ...
    Btc1dOneBarFailedDipContinuationHighCagrConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d one-bar failed dip continuation high-CAGR batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dOneBarFailedDipContinuationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dOneBarFailedDipContinuationHighCagrBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
