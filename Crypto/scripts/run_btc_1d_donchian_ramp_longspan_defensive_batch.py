...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d donchian ramp longspan defensive batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dDonchianRampLongspanDefensiveConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dDonchianRampLongspanDefensiveBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
