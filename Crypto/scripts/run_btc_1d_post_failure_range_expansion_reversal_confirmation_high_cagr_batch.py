...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the BTC 1d post-failure range expansion reversal confirmation high-CAGR batch."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dPostFailureRangeExpansionReversalConfirmationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    service = Btc1dPostFailureRangeExpansionReversalConfirmationHighCagrBatchService(analysis_results_dir=args.analysis_dir)
    result = service.run_batch(config)
