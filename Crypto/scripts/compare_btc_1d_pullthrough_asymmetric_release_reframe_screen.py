...
def _latest_batch_path(analysis_dir: Path) -> Path:
    matches = sorted(
        analysis_dir.glob("btc_1d_pullthrough_asymmetric_release_reframe_batch_*.json"),
        key=lambda path: path.name,
    )
    if not matches:
        raise FileNotFoundError("No pullthrough asymmetric release reframe batch artifact found.")
    return matches[-1]
...
def build_report_from_batch(batch: dict) -> dict:
    ...
    return {
        ...
        "reframe_verdict": {
            ...
            "next_step": "candidate_stage_followup" if ready else "continue_reframe_search",
            "reason": (
                "The pullthrough asymmetric release reframe produced a clean candidate-stage family."
                if ready
                else "The pullthrough asymmetric release reframe still did not clear the candidate-stage bar."
            ),
        },
        "decision_summary": [
            (
                f"Best reframe variant is `{best['variant_label']}` with `{float(best['cagr']):.4f}` CAGR, "
                f"`{float(best['max_drawdown']):.4f}` MDD, and `{int(best['trades'])}` trades."
            ),
            "Advance only if the pullthrough reframe can cross candidate-stage without losing clean stage2 behavior.",
        ],
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare pullthrough asymmetric release reframe batch results.")
    ...
    json_path = args.analysis_dir / f"btc_1d_pullthrough_asymmetric_release_reframe_screen_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_pullthrough_asymmetric_release_reframe_screen_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    md_lines = [
        "# BTC 1d Pullthrough Asymmetric Release Reframe Screen",
        "",
        ...
        f"- Has candidate-stage seed: `{report['reframe_verdict']['has_candidate_stage_seed']}`",
        f"- Next step: `{report['reframe_verdict']['next_step']}`",
        f"- Reason: {report['reframe_verdict']['reason']}",
        "",
    ]
