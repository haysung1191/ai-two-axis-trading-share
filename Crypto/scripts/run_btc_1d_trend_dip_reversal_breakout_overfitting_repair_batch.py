...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTC 1d trend dip reversal breakout overfitting-repair batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dTrendDipReversalBreakoutOverfittingRepairConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dTrendDipReversalBreakoutOverfittingRepairBatchService(
