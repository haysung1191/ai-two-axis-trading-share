...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run BTCUSDT 1d paper validation for the volatility expansion pullthrough softer-setup hold-31 candidate."
    )
...
def parse_args(argv: list[str] | None = None) -> BtcPaperValidationConfig:
    ...
    return BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d volatility expansion pullthrough softer-setup hold-31 candidate aims to preserve the repaired attack profile while keeping drawdown and drift controlled.",
        ema_fast_window=20,
        ema_slow_window=50,
        atr_window=14,
        atr_multiple=3.5,
