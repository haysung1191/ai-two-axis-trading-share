...
from app.domains.experiments.btc_1d_shallow_breakout_shelf_continuation_high_cagr_batch import (
    ...
    Btc1dShallowBreakoutShelfContinuationHighCagrConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d shallow breakout shelf continuation high-CAGR batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dShallowBreakoutShelfContinuationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dShallowBreakoutShelfContinuationHighCagrBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
