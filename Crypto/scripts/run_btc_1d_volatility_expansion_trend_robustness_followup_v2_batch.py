...
from app.domains.experiments.btc_1d_volatility_expansion_trend_robustness_followup_v2_batch import (
    Btc1dVolatilityExpansionTrendRobustnessFollowupV2BatchService,
    Btc1dVolatilityExpansionTrendRobustnessFollowupV2Config,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d volatility expansion trend robustness follow-up v2 batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = Btc1dVolatilityExpansionTrendRobustnessFollowupV2BatchService(analysis_results_dir=args.analysis_dir).run_batch(
        config
    )
