...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTCUSDT 1d paper validation for the practical BTC-only candidate.")
...
def parse_args(argv: list[str] | None = None) -> BtcPaperValidationConfig:
    ...
    artifact_label = f"{str(args.symbol).lower()}_{str(args.interval).lower()}_{int(args.periods)}"
    return BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d volatility expansion reclaim defensive variant lowers drawdown via lower ATR window and tighter stop while preserving high-30s CAGR.",
        ema_fast_window=20,
        ema_slow_window=50,
        atr_window=14,
        atr_multiple=3.5,
