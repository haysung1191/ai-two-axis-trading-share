...
from scripts.validate_btc_1d_volatility_expansion_reclaim_cooldown_floor_candidate import parse_args as parse_floor_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d volatility expansion reclaim cooldown floor candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d volatility expansion reclaim cooldown floor candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "cooldown floor candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "cooldown floor candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "cooldown floor candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "cooldown floor candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Volatility Expansion Reclaim Cooldown Floor Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "candidate": "volatility_expansion_reclaim_cooldown12_slope_floor",
    ...
    json_path = args.analysis_dir / f"btc_1d_volatility_expansion_reclaim_cooldown_floor_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_volatility_expansion_reclaim_cooldown_floor_friction_{stamp}.md"
