...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d low vol regime breakout longspan defensive batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dLowVolRegimeBreakoutLongspanDefensiveConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dLowVolRegimeBreakoutLongspanDefensiveBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
