...
from scripts.compare_btc_1d_pullthrough_asymmetric_release_live_readiness_review import (
    build_report as build_live_readiness_review_report,
)
...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    live_readiness_review = build_live_readiness_review_report(
    ...
    context = dict(live_readiness_review.get("stack_context", {}) or {})
    candidate = dict(live_readiness_review.get("candidate_profile", {}) or {})
    review_requirements = dict(
        live_readiness_review.get(
    ...
    activation_requirements = {
        "challenger_candidate_live_readiness_review_ready": bool(
            review_verdict.get("challenger_candidate_live_readiness_review_ready", False)
        ),
        "operator_verdict_shadow_monitoring_ready": (
            str(operating_index.get("operator_verdict", "")) == "shadow_monitoring_ready"
        ),
        "operating_index_live_readiness_review_ready": bool(
            operating_index.get("attack_challenger_live_readiness_review_ready", False)
        ),
        "operating_brief_live_readiness_review_ready": bool(
            operating_brief.get("attack_challenger_live_readiness_review_ready", False)
        ),
        "dashboard_live_readiness_review_ready": bool(
        ...
        "queue_lane_mirrored": (
            ...
            == str(
                operating_brief.get("attack_challenger_live_readiness_review_lane", "")
            )
            == str(
    ...
    activation_ready = all(activation_requirements.values())
    activation_lane = (
    ...
    next_step_now = (
        "challenger_live_candidate_entry"
        if activation_ready
        else "repair_challenger_live_shadow_activation_review"
    )
    ...
    return {
        ...
        "challenger_live_shadow_activation_review_verdict": {
            "challenger_live_shadow_activation_review_ready": activation_ready,
            "challenger_live_shadow_activation_review_lane": activation_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Live-readiness review is green and every operator-facing mirror agrees, so the challenger can enter live shadow activation review."
                if activation_ready
                else "The challenger cleared live-readiness review, but live-shadow-activation mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger live-shadow-activation candidate.",
            (
                "Advance to `challenger_live_candidate_entry` because live-readiness review and operator-facing mirrors are all green."
                if activation_ready
                else "Do not advance into live-shadow activation review until all activation-review requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes live-shadow activation review.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Shadow Activation Review",
            "",
            ...
            f"- Challenger live shadow activation review ready: `{verdict['challenger_live_shadow_activation_review_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_shadow_activation_review_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_activation_review_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_activation_review_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
    ...
    latest_md = (
        ANALYSIS_DIR
        / "btc_1d_pullthrough_asymmetric_release_live_shadow_activation_review_md_latest.md"
    )
