...
from scripts.compare_btc_1d_pullthrough_asymmetric_release_live_shadow_activation_review import (
    build_report as build_live_shadow_activation_review_report,
)
...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    activation_review = build_live_shadow_activation_review_report(
    ...
    context = dict(activation_review.get("stack_context", {}) or {})
    candidate = dict(activation_review.get("candidate_profile", {}) or {})
    review_requirements = dict(
        activation_review.get("challenger_live_shadow_activation_review_requirements", {})
        or {}
    )
    review_verdict = dict(
        activation_review.get("challenger_live_shadow_activation_review_verdict", {})
        or {}
    )
    dashboard_summary = dict(dashboard.get("dashboard_summary", {}) or {})

    candidate_entry_requirements = {
        "challenger_live_shadow_activation_review_ready": bool(
            review_verdict.get("challenger_live_shadow_activation_review_ready", False)
        ),
        "operator_verdict_shadow_monitoring_ready": (
            str(operating_index.get("operator_verdict", "")) == "shadow_monitoring_ready"
        ),
        "operating_index_live_shadow_activation_review_ready": bool(
            operating_index.get(
                "attack_challenger_live_shadow_activation_review_ready", False
            )
        ),
        "operating_brief_live_shadow_activation_review_ready": bool(
            operating_brief.get(
                "attack_challenger_live_shadow_activation_review_ready", False
            )
        ),
        "dashboard_live_shadow_activation_review_ready": bool(
    ...
    candidate_entry_ready = all(candidate_entry_requirements.values())
    candidate_entry_lane = (
        "challenger_live_candidate_queue"
        if candidate_entry_ready
        else "challenger_live_candidate_repair_hold"
    )
    next_step_now = (
        "challenger_live_operator_paper_entry"
        if candidate_entry_ready
        else "repair_challenger_live_candidate_entry"
    )
    ...
    return {
        ...
        "challenger_live_candidate_entry_requirements": candidate_entry_requirements,
        "challenger_live_candidate_entry_verdict": {
            "challenger_live_candidate_entry_ready": candidate_entry_ready,
            "challenger_live_candidate_entry_lane": candidate_entry_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Live-shadow activation review is green and every operator-facing mirror agrees, so the challenger can enter live candidate entry."
                if candidate_entry_ready
                else "The challenger cleared live-shadow activation review, but live-candidate-entry mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger live-candidate-entry candidate.",
            (
                "Advance to `challenger_live_operator_paper_entry` because live-shadow activation review and operator-facing mirrors are all green."
                if candidate_entry_ready
                else "Do not advance into live candidate entry until all candidate-entry requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes live-candidate entry.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    requirements = report["challenger_live_candidate_entry_requirements"]
    verdict = report["challenger_live_candidate_entry_verdict"]
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Candidate Entry",
            "",
            ...
            f"- Challenger live candidate entry ready: `{verdict['challenger_live_candidate_entry_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_candidate_entry_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_candidate_entry_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_candidate_entry_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
    ...
    latest_md = (
        ANALYSIS_DIR
        / "btc_1d_pullthrough_asymmetric_release_live_candidate_entry_md_latest.md"
    )
