...
from scripts.compare_btc_1d_pullthrough_asymmetric_release_live_shadow_candidate_paper_review import (
    build_report as build_live_shadow_candidate_paper_review_report,
)
...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    candidate_paper_review = build_live_shadow_candidate_paper_review_report(
    ...
    context = dict(candidate_paper_review.get("stack_context", {}) or {})
    candidate = dict(candidate_paper_review.get("candidate_profile", {}) or {})
    review_requirements = dict(
        candidate_paper_review.get(
    ...
    governance_lock_requirements = {
        "challenger_live_shadow_candidate_paper_review_ready": bool(
            review_verdict.get("challenger_live_shadow_candidate_paper_review_ready", False)
        ),
        "operator_verdict_shadow_monitoring_ready": (
            str(operating_index.get("operator_verdict", "")) == "shadow_monitoring_ready"
        ),
        "operating_index_live_shadow_candidate_paper_review_ready": bool(
            operating_index.get(
                "attack_challenger_live_shadow_candidate_paper_review_ready", False
            )
        ),
        "operating_brief_live_shadow_candidate_paper_review_ready": bool(
            operating_brief.get(
                "attack_challenger_live_shadow_candidate_paper_review_ready", False
            )
        ),
        "dashboard_live_shadow_candidate_paper_review_ready": bool(
    ...
    governance_lock_ready = all(governance_lock_requirements.values())
    governance_lock_lane = (
    ...
    next_step_now = (
        "challenger_live_shadow_locked_entry"
        if governance_lock_ready
        else "repair_challenger_live_shadow_candidate_governance_lock"
    )
    ...
    return {
        ...
        "challenger_live_shadow_candidate_governance_lock_requirements": governance_lock_requirements,
        "challenger_live_shadow_candidate_governance_lock_verdict": {
            "challenger_live_shadow_candidate_governance_lock_ready": governance_lock_ready,
            "challenger_live_shadow_candidate_governance_lock_lane": governance_lock_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Shadow candidate paper review is green and every operator-facing mirror agrees, so the challenger can enter governance lock."
                if governance_lock_ready
                else "The challenger cleared shadow candidate paper review, but the governance-lock mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger shadow candidate governance-lock target.",
            (
                "Advance to `challenger_live_shadow_locked_entry` because shadow candidate paper review and operator-facing mirrors are all green."
                if governance_lock_ready
                else "Do not advance into governance lock until all governance-lock requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes governance lock.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    requirements = report["challenger_live_shadow_candidate_governance_lock_requirements"]
    verdict = report["challenger_live_shadow_candidate_governance_lock_verdict"]
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Shadow Candidate Governance Lock",
            "",
            ...
            f"- Challenger live shadow candidate governance lock ready: `{verdict['challenger_live_shadow_candidate_governance_lock_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_shadow_candidate_governance_lock_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_candidate_governance_lock_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_candidate_governance_lock_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
    ...
    latest_md = (
        ANALYSIS_DIR
        / "btc_1d_pullthrough_asymmetric_release_live_shadow_candidate_governance_lock_md_latest.md"
    )
