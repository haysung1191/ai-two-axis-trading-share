...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTC 1d one-bar failed dip continuation reopen batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dOneBarFailedDipContinuationReopenConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dOneBarFailedDipContinuationReopenBatchService(
