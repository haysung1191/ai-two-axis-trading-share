...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run concentration diagnostics for the BTC practical leader.")
...
def parse_args(argv: list[str] | None = None) -> ConcentrationConfig:
...
def _load_ohlcv(config: ConcentrationConfig) -> pd.DataFrame:
...
def _compute_returns_and_trades(
    ...
    *,
    ...
    for idx, timestamp in enumerate(ohlcv.index):
...
def _monthly_concentration(strategy_returns: pd.Series) -> dict[str, Any]:
    ...
    top_months = [
        ...
        for timestamp, value in positive.head(5).items()
    ]
    return {
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# BTC Practical Leader Concentration",
        "",
        ...
        "## Trades",
        "",
        f"- completed trades: `{report['trade_concentration']['completed_trades']}`",
        f"- positive trades: `{report['trade_concentration']['positive_trades']}`",
        f"- top 1 trade share: `{report['trade_concentration']['top_1_trade_share']:.4f}`",
        f"- top 3 trade share: `{report['trade_concentration']['top_3_trade_share']:.4f}`",
        f"- top 5 trade share: `{report['trade_concentration']['top_5_trade_share']:.4f}`",
        "",
        "## Months",
        "",
        f"- top 1 month share: `{report['monthly_concentration']['top_1_month_share']:.4f}`",
        f"- top 3 month share: `{report['monthly_concentration']['top_3_month_share']:.4f}`",
        f"- top 5 month share: `{report['monthly_concentration']['top_5_month_share']:.4f}`",
    ]
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = analysis_dir / f"btc_1d_volatility_expansion_reclaim_stab_atrs_concentration_{stamp}.json"
    md_path = analysis_dir / f"btc_1d_volatility_expansion_reclaim_stab_atrs_concentration_{stamp}.md"
