...
from app.domains.experiments.btc_1d_brief_close_above_reset_continuation_high_cagr_batch import (
    ...
    Btc1dBriefCloseAboveResetContinuationHighCagrConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d brief close above reset continuation high-CAGR batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Btc1dBriefCloseAboveResetContinuationHighCagrConfig(
        periods=args.periods,
        fee_bps=args.fee_bps,
        slippage_bps=args.slippage_bps,
        allow_synthetic_ohlcv_fallback=bool(args.allow_synthetic_ohlcv_fallback),
    )
    result = Btc1dBriefCloseAboveResetContinuationHighCagrBatchService(analysis_results_dir=args.analysis_dir).run_batch(config)
