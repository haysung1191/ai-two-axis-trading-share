...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BTC 1d practical benchmark comparison batch.")
...
def parse_args(argv: list[str] | None = None) -> Btc1dPracticalBenchmarkConfig:
    args = build_parser().parse_args(argv)
    return Btc1dPracticalBenchmarkConfig(
        symbol=args.symbol,
...
def main(argv: list[str] | None = None) -> int:
    result = Btc1dPracticalBenchmarkBatchService().run_batch(parse_args(argv))
