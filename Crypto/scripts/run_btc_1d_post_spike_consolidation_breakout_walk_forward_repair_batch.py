...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run BTC 1d post-spike consolidation breakout walk-forward repair batch."
    )
    ...
    parser.add_argument("--variant-label", action="append", dest="variant_labels", default=[])
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = service.run_batch(
        Btc1dPostSpikeConsolidationBreakoutWalkForwardRepairConfig(
            ...
            variant_labels=tuple(args.variant_labels),
        )
    )
