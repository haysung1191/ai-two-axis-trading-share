...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    ...
    context = dict(governance_check.get("stack_context", {}) or {})
    candidate = dict(governance_check.get("candidate_profile", {}) or {})
    prior_requirements = dict(
        governance_check.get(
    ...
    governance_entry_requirements = {
        "challenger_live_shadow_locked_release_governance_check_ready": bool(
            prior_verdict.get(
        ...
        "operating_index_live_shadow_locked_release_governance_check_ready": bool(
            operating_index.get(
                "attack_challenger_live_shadow_locked_release_governance_check_ready",
                False,
            )
        ),
        "operating_brief_live_shadow_locked_release_governance_check_ready": bool(
            operating_brief.get(
                "attack_challenger_live_shadow_locked_release_governance_check_ready",
                False,
            )
        ),
        "dashboard_live_shadow_locked_release_governance_check_ready": bool(
    ...
    governance_entry_ready = all(governance_entry_requirements.values())
    governance_entry_lane = (
        "challenger_live_shadow_locked_release_governance_entry_queue"
        if governance_entry_ready
        else "challenger_live_shadow_locked_release_governance_entry_repair_hold"
    )
    next_step_now = (
        "remote monitoring and deployment handoff"
        if governance_entry_ready
        else "repair_challenger_live_shadow_locked_release_governance_entry"
    )
    ...
    return {
        ...
        "challenger_live_shadow_locked_release_governance_entry_requirements": governance_entry_requirements,
        "challenger_live_shadow_locked_release_governance_entry_verdict": {
            "challenger_live_shadow_locked_release_governance_entry_ready": governance_entry_ready,
            "challenger_live_shadow_locked_release_governance_entry_lane": governance_entry_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Locked release governance check is green and every operator-facing mirror agrees, so the challenger can complete locked release governance entry and hand off to deployment monitoring."
                if governance_entry_ready
                else "The challenger cleared locked release governance check, but governance-entry mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger locked-release governance-entry candidate.",
            (
                "Advance to `remote monitoring and deployment handoff` because locked release governance check and operator-facing mirrors are all green."
                if governance_entry_ready
                else "Do not advance into locked release governance entry until all governance-entry requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes locked release governance entry.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    verdict = report["challenger_live_shadow_locked_release_governance_entry_verdict"]
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Shadow Locked Release Governance Entry",
            "",
            ...
            f"- Challenger live shadow locked release governance entry ready: `{verdict['challenger_live_shadow_locked_release_governance_entry_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_shadow_locked_release_governance_entry_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_governance_entry_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_shadow_locked_release_governance_entry_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
