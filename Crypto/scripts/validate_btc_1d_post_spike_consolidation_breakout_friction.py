...
from scripts.post_spike_active_candidate import ACTIVE_CANDIDATE_LABEL
from scripts.validate_btc_1d_post_spike_consolidation_breakout_candidate import parse_args as parse_candidate_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the active BTCUSDT 1d post-spike consolidation breakout candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis=f"BTCUSDT 1d {ACTIVE_CANDIDATE_LABEL} candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", f"{ACTIVE_CANDIDATE_LABEL} fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", f"{ACTIVE_CANDIDATE_LABEL} remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", f"{ACTIVE_CANDIDATE_LABEL} survives moderate friction but weakens before the heaviest level."
    return "caution", f"{ACTIVE_CANDIDATE_LABEL} survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Post Spike Consolidation Breakout Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "candidate": ACTIVE_CANDIDATE_LABEL,
    ...
    json_path = args.analysis_dir / f"btc_1d_post_spike_consolidation_breakout_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_post_spike_consolidation_breakout_friction_{stamp}.md"
    latest_json = args.analysis_dir / "btc_1d_post_spike_consolidation_breakout_friction_latest.json"
    latest_md = args.analysis_dir / "btc_1d_post_spike_consolidation_breakout_friction_latest.md"
