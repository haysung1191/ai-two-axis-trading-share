...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run BTCUSDT 1d paper validation for the volatility expansion reclaim ratio-111 tighter-stop candidate."
    )
...
def parse_args(argv: list[str] | None = None) -> BtcPaperValidationConfig:
    ...
    return BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d volatility expansion reclaim ratio-111 tighter-stop candidate aims to preserve most of the new attack leader while slightly reducing ATR-threshold sensitivity.",
        ema_fast_window=20,
        ema_slow_window=50,
        atr_window=14,
        atr_multiple=3.5,
