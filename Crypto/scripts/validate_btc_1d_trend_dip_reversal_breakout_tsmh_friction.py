...
from scripts.validate_btc_1d_trend_dip_reversal_breakout_tsmh_candidate import parse_args as parse_candidate_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d trend dip reversal breakout tighter-stop mid-hold candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d trend dip reversal breakout tighter-stop mid-hold candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "trend dip reversal breakout tighter-stop mid-hold candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "trend dip reversal breakout tighter-stop mid-hold candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "trend dip reversal breakout tighter-stop mid-hold candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "trend dip reversal breakout tighter-stop mid-hold candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Trend Dip Reversal Breakout Tighter Stop Mid Hold Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "candidate": "trend_dip_reversal_breakout_tighter_stop_mid_hold",
    ...
    json_path = args.analysis_dir / f"btc_1d_trend_dip_reversal_breakout_tsmh_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_trend_dip_reversal_breakout_tsmh_friction_{stamp}.md"
