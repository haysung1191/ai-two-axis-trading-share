...
from scripts.validate_btc_1d_shallow_liquidity_void_refill_candidate import parse_args as parse_candidate_args


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run friction sanity check for the BTCUSDT 1d shallow liquidity void refill continuation candidate."
    )
...
def _run_validation(
    *,
    ...
    cfg = BtcPaperValidationConfig(
        ...
        hypothesis="BTCUSDT 1d shallow liquidity void refill continuation candidate under friction sanity check.",
...
def _decision(levels: list[dict]) -> tuple[str, str]:
    ...
    if not surviving:
        return "pause", "shallow liquidity void refill continuation candidate fails under all tested friction assumptions."
    max_surviving = max(level["cost_bps"] for level in surviving)
    if max_surviving >= 20.0:
        return "continue", "shallow liquidity void refill continuation candidate remains paper-valid even under the heaviest tested friction."
    if max_surviving >= 12.0:
        return "continue_with_caution", "shallow liquidity void refill continuation candidate survives moderate friction but weakens before the heaviest level."
    return "caution", "shallow liquidity void refill continuation candidate survives only under lighter friction assumptions."


def _render_markdown(report: dict) -> str:
    lines = [
        "# BTC 1d Shallow Liquidity Void Refill Continuation Friction Check",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "candidate": "shallow_liquidity_void_refill_continuation_reference",
    ...
    json_path = args.analysis_dir / f"btc_1d_shallow_liquidity_void_refill_friction_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_shallow_liquidity_void_refill_friction_{stamp}.md"
