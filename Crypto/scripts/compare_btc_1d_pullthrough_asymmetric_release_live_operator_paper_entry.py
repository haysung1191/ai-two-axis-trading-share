...
from scripts.compare_btc_1d_pullthrough_asymmetric_release_live_candidate_entry import (
    build_report as build_live_candidate_entry_report,
)
...
def build_report(analysis_dir: Path = ANALYSIS_DIR) -> dict:
    candidate_entry = build_live_candidate_entry_report(analysis_dir=analysis_dir)
    ...
    entry_requirements = dict(
        candidate_entry.get("challenger_live_candidate_entry_requirements", {}) or {}
    )
    entry_verdict = dict(
        candidate_entry.get("challenger_live_candidate_entry_verdict", {}) or {}
    )
    dashboard_summary = dict(dashboard.get("dashboard_summary", {}) or {})

    operator_paper_requirements = {
        "challenger_live_candidate_entry_ready": bool(
            entry_verdict.get("challenger_live_candidate_entry_ready", False)
        ),
        "operator_verdict_shadow_monitoring_ready": (
            str(operating_index.get("operator_verdict", "")) == "shadow_monitoring_ready"
        ),
        "operating_index_live_candidate_entry_ready": bool(
            operating_index.get("attack_challenger_live_candidate_entry_ready", False)
        ),
        "operating_brief_live_candidate_entry_ready": bool(
            operating_brief.get("attack_challenger_live_candidate_entry_ready", False)
        ),
        "dashboard_live_candidate_entry_ready": bool(
        ...
        "queue_lane_mirrored": (
            ...
            == str(
                operating_brief.get("attack_challenger_live_candidate_entry_lane", "")
            )
            == str(
    ...
    operator_paper_ready = all(operator_paper_requirements.values())
    operator_paper_lane = (
    ...
    next_step_now = (
        "challenger_live_shadow_governance_review"
        if operator_paper_ready
        else "repair_challenger_live_operator_paper_entry"
    )
    ...
    return {
        ...
        "challenger_live_operator_paper_entry_requirements": operator_paper_requirements,
        "challenger_live_operator_paper_entry_verdict": {
            "challenger_live_operator_paper_entry_ready": operator_paper_ready,
            "challenger_live_operator_paper_entry_lane": operator_paper_lane,
            "next_step_now": next_step_now,
            "reason": (
                "Live candidate entry is green and every operator-facing mirror agrees, so the challenger can enter live operator paper entry."
                if operator_paper_ready
                else "The challenger cleared live candidate entry, but live operator paper entry mirrors are not fully aligned yet."
            ),
        },
        "decision_summary": [
            f"Treat `{context.get('attack_challenger_candidate', candidate.get('label', ''))}` as the active challenger live-operator-paper-entry candidate.",
            (
                "Advance to `challenger_live_shadow_governance_review` because live-candidate entry and operator-facing mirrors are all green."
                if operator_paper_ready
                else "Do not advance into live operator paper entry until all operator-paper-entry requirements are green."
            ),
            f"Keep `{context.get('attack_main', '')}` and `{context.get('attack_backup', '')}` as references while the challenger completes live-operator-paper entry.",
        ],
    }
...
def _render_markdown(report: dict) -> str:
    ...
    return "\n".join(
        [
            "# BTC 1d Pullthrough Asymmetric Release Challenger Live Operator Paper Entry",
            "",
            ...
            f"- Challenger live operator paper entry ready: `{verdict['challenger_live_operator_paper_entry_ready']}`",
            f"- Queue lane: `{verdict['challenger_live_operator_paper_entry_lane']}`",
...
def main() -> int:
    ...
    json_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_operator_paper_entry_{stamp}.json"
    )
    md_path = (
        ANALYSIS_DIR
        / f"btc_1d_pullthrough_asymmetric_release_live_operator_paper_entry_{stamp}.md"
    )
    latest_json = (
        ANALYSIS_DIR
    ...
    latest_md = (
        ANALYSIS_DIR
        / "btc_1d_pullthrough_asymmetric_release_live_operator_paper_entry_md_latest.md"
    )
