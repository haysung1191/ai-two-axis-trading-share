...
from app.domains.experiments.btc_1d_volatility_expansion_trend_atr_ratio_stabilization_batch import (
    Btc1dVolatilityExpansionTrendAtrRatioStabilizationBatchService,
    Btc1dVolatilityExpansionTrendAtrRatioStabilizationConfig,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BTC 1d volatility expansion trend ATR-ratio stabilization batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = Btc1dVolatilityExpansionTrendAtrRatioStabilizationBatchService(analysis_results_dir=args.analysis_dir).run_batch(
        config
    )
