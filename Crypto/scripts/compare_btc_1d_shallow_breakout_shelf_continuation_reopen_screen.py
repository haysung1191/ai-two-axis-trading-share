...
def _latest_batch_path(analysis_dir: Path) -> Path:
    matches = sorted(
        analysis_dir.glob("btc_1d_shallow_breakout_shelf_continuation_reopen_batch_*.json"),
        key=lambda path: path.name,
    )
    if not matches:
        raise FileNotFoundError("No shallow breakout shelf continuation reopen batch artifact found.")
    return matches[-1]
...
def build_report_from_batch(batch: dict) -> dict:
    ...
    return {
        ...
        "reopen_verdict": {
            ...
            "reason": (
                "The compression-reset seed reopened into a candidate-stage family with enough CAGR, trades, and drawdown control to justify follow-up."
                if ready
                else "The compression-reset seed still cannot clear the candidate-stage bar, so the broad family search should continue."
            ),
        },
        "decision_summary": [
            (
            ...
            "Advance only if the reopen batch can lift this family above the candidate-stage floor without breaking the low-drawdown compression-reset profile.",
        ],
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compare shallow breakout shelf continuation reopen batch results.")
    ...
    json_path = args.analysis_dir / f"btc_1d_shallow_breakout_shelf_continuation_reopen_screen_{stamp}.json"
    md_path = args.analysis_dir / f"btc_1d_shallow_breakout_shelf_continuation_reopen_screen_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    md_lines = [
        "# BTC 1d Shallow Breakout Shelf Continuation Reopen Screen",
        "",
